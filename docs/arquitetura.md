...
4. Dentro de `semantica()`, o texto é convertido em um vetor de 1536 dimensões pelo modelo `gemini-embedding-001` com `task_type='RETRIEVAL_QUERY'`.
5. O vetor é passado para `recuperar_contexto_inteligente()` em `src/core/database.py` (Facade que re-exporta a lógica de `src/core/db/retrieval.py`), que executa a busca vetorial no Supabase via RPC (função `match_knowledge_base`).
...
7. O contexto recuperado é combinado com o prompt original e enviado para `gerar_resposta()` em `genai.py`.
8. O Gemini gera a resposta em streaming, exibida letra por letra na interface.
9. O log (prompt, resposta, IDs dos chunks usados, versão git) é salvo no Supabase.
